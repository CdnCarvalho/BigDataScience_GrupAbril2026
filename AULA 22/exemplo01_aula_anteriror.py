...
try:
    ...
    for i, valor in enumerate(df_roubo_veiculo_outliers_superiores['roubo_veiculo']):
        ...
        # usamos os próprios valores dos roubos para dar posição, vertical dos valores.
        plt.text(
            # i e o valor, são usados como coordenadas q dão a posição dos valores no gráfico
            i,  # posição X 
            valor + deslocamento,  # posição Y (soma o deslocamento para afastar o texto)
            f'{valor:,}',
            ha='center'
        )
    ...
    if len(df_roubo_veiculo_outliers_inferiores) > 0:
        ...
        # enumerate recupera os valores dos roubos e a posição de cada um na lista
        # No gráfico de Barras usamos a posição em Y, que é i, p/ identificar cada barra. E
        # usamos os próprios valores dos roubos para posicionar os valores na horizontal.
            plt.text(
                valor + deslocamento,  # posição X (soma o deslocamento para afastar o txto)
                i,  # posição Y
                f'{valor:,}',
                ha='center'
            )
    ...
    # Alterar esta parte
    plt.suptitle('Roubos de Veículos por Município', fontsize=16, fontweight='bold')  # Título do Painel
    plt.tight_layout()  # ajusta o layout do gráfico para que não fique cortado, sobreposto ou muito apertado
    plt.show()
