...
try:
    ...
    # e computacionalmente é mais eficiente para cálculo estatísticos
    ...
    # Se a média for muito diferente da mediana, tende a uma assimetria forte. Não tende a haver um padrão
    # Se a distância for menor que 10%, a distribuição tende a ser simétrica. Tende a haver um padrão.
    # Se a distância for entre 10% a 25% a distribuição tende a ter uma assimetria moderada. Valores extremos podem estar influenciando na média. 
    # Se a distância for maior que 25%, distribuição tende a ser assimétrica forte. Valores extremos podem estar influenciando na média. 
    ...
    q1 = np.quantile(array_roubo_veiculo, 0.25) # Q1 é 25% , method='weibull'
    q2 = np.quantile(array_roubo_veiculo, 0.50) # Q2 é 50% (mediana) , method='weibull'
    q3 = np.quantile(array_roubo_veiculo, 0.75) # Q3 é 75%  , method='weibull'
